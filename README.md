...
3. **Deploying Kong on the cluster**
    ...
    Using `kong_<cassandra|postgres>.yaml`, deploy a  Kong `Service` and a `Deployment` to the cluster created in the last step.
    
    When ready, deploy with the following command:

    ```bash
    $ kubectl create -f kong_<cassandra|postgres>.yaml
    ```
